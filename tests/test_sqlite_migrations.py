...
def test_initial_migrations_have_one_global_continuous_history():
    ...
    assert [migration.label for migration in migrations] == [
        ...
        "0013_add_flasks",
        "0014_add_free_equipment_chance",
        "0015_add_clan_technologies",
    ]
    assert all(
...
def test_game_account_migration_builds_expected_schema_and_constraints(tmp_path):
    ...
    with sqlite3.connect(database_path) as connection:
        ...
        account_columns = connection.execute(
            "PRAGMA table_info(game_accounts)"
        ).fetchall()
    ...
    assert [column[1] for column in columns] == [
        ...
        *MIGRATED_DATA_COLUMNS,
        ...
        "free_equipment_chance_updated_on",
    ]
    assert columns[0][5] == 1
    assert all(column[3] == 1 for column in columns[1:])
    clan_id_column = next(
        column for column in account_columns if column[1] == "clan_id"
    )
    assert clan_id_column[3] == 0
    assert any(
