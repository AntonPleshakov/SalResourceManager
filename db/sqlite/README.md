...
`release_views.py`, `user_data.py`, and `war_stages.py`. Shared connection and
transaction handling live in `database.py`.
...
the application starts without additional schema validation or comparison with
Google Sheets. During migration stage 2, reads use SQLite while writes update
Google first and SQLite second.
